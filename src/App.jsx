// src/App.jsx — v3: iPhone safe area + mobile layout fix
  ...
  return (
    ...
    >
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        >
          ¿Olvidaste tu contraseña? Pedísela a mamá o papá 😄
        </p>
      </div>
    </div>
  );
...
function ManageCategoriesModal({ onClose, groups, categories, onRefresh }) {
  ...
  async function saveGroup() {
    ...
    await supabase
      ...
      .insert([{ name: groupName.trim(), icon: groupIcon, color: groupColor }]);
    await onRefresh();
    setGroupName("");
    setGroupIcon("📁");
    setGroupColor(PALETTE[0]);
    setView("list");
    setLoading(false);
  }
  ...
  async function saveCat() {
    ...
    await supabase
      .from("categories")
      .insert([{ name: catName.trim(), icon: catIcon, group_id: selectedGroup.id }]);
    await onRefresh();
    setCatName("");
    setCatIcon("📦");
    setView("list");
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            >
              {view === "list"
                ...
                : view === "newGroup"
                ? "Nueva categoría"
                : `Subcategoría en ${selectedGroup?.name}`}
            </h2>
          </div>
          <button
            onClick={onClose}
            ...
            <div>
              ...
              {groups.map((g) => {
                ...
                async function toggleExclude() {
                  await supabase
                    .from("category_groups")
                    .update({ exclude_from_ranking: !g.exclude_from_ranking })
                    .eq("id", g.id);
                  await onRefresh();
                }
                  ...
                  >
                    <div
                      style={{
                        ...
                        gap: 10,
                        padding: "14px 16px",
                        borderBottom: gCats.length > 0 ? `1px solid ${C.border}` : "none"
                      }}
                    >
                      <div
                        style={{
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          <div style={{ fontSize: 11, color: C.muted }}>
                            {gCats.length} subcategoría{gCats.length !== 1 ? "s" : ""}
                          </div>
                          {g.exclude_from_ranking && (
                            <span
                              style={{
                                fontSize: 10,
                      ...
                      <button
                        ...
                        style={{
                          background: g.exclude_from_ranking ? C.yellow + "22" : C.card,
                          border: `1px solid ${
                            g.exclude_from_ranking ? C.yellow : C.border
                          }`,
                          color: g.exclude_from_ranking ? C.yellow : C.muted,
                          borderRadius: 8,
                          padding: "5px 8px",
                          fontSize: 14,
                          cursor: "pointer",
                    ...
                    {gCats.map((c, i) => (
                      <div
                        ...
                        style={{
                          ...
                          gap: 10,
                          padding: "11px 16px",
                          borderBottom: i < gCats.length - 1 ? `1px solid ${C.border}` : "none",
              ...
              {groups.length > 0 && (
                ...
                >
                  💡 Tocá <strong>🏆</strong> en una categoría para excluirla del
                  ranking. Útil para gastos compartidos del hogar.
                </div>
              )}
            </div>
          )}
            ...
            <div>
              <div style={{ marginBottom: 20 }}>
                ...
                <input
                  ...
                  placeholder="Ej: Casa, Auto, Salud..."
                  style={{
                    width: "100%",
                    border: `1.5px solid ${C.border}`,
                    borderRadius: 12,
                ...
                >
                  {ICONS.map((ic) => (
                    <button
                      ...
                      style={{
                        background: groupIcon === ic ? C.accentSoft : C.card,
                        border: `1.5px solid ${groupIcon === ic ? C.accent : C.border}`,
                        borderRadius: 8,
                        padding: "8px 4px",
                        cursor: "pointer",
                        fontSize: 18,
                  ...
                  {PALETTE.map((col) => (
                    <button
                      ...
                      style={{
                        ...
                        background: col,
                        borderRadius: "50%",
                        border: groupColor === col ? `3px solid ${C.text}` : "3px solid transparent",
                        cursor: "pointer",
                        boxShadow: groupColor === col ? `0 0 0 2px ${col}` : "none"
                      }}
                    />
                  ))}
                </div>
              ...
              {groupName && (
                ...
                >
                  <span style={{ fontSize: 24 }}>{groupIcon}</span>
                ...
                >
                  {ICONS.map((ic) => (
                    <button
                      ...
                      style={{
                        background: catIcon === ic ? C.accentSoft : C.card,
                        border: `1.5px solid ${catIcon === ic ? C.accent : C.border}`,
                        borderRadius: 8,
                        padding: "8px 4px",
                        cursor: "pointer",
                        fontSize: 18,
  ...
  async function handleSubmit() {
    ...
    await onAdd({
      ...
      category: selectedCat
        ? `${selectedGroup.name} › ${selectedCat.name}`
        : selectedGroup.name,
          ...
          <>
            ...
            <div style={{ overflowY: "auto", flex: 1, padding: "20px 24px 32px" }}>
              {groups.length === 0 ? (
                <div style={{ textAlign: "center", padding: "30px 0", color: C.muted }}>
                  <div style={{ fontSize: 40, marginBottom: 10 }}>📂</div>
                  <p>Primero creá categorías usando el botón ⚙️.</p>
                </div>
              ) : (
                <>
                  <div style={{ marginBottom: 20 }}>
                    ...
                    >
                      {groups.map((g) => (
                        <button
                          ...
                          style={{
                            background: selectedGroup?.id === g.id ? g.color + "33" : C.card,
                            border: `1.5px solid ${
                              selectedGroup?.id === g.id ? g.color : C.border
                            }`,
                            borderRadius: 12,
                            padding: "12px 8px",
                            cursor: "pointer",
                        ...
                        >
                          ...
                          <span
                            style={{
                              fontSize: 11,
                              color: selectedGroup?.id === g.id ? g.color : C.muted,
                              fontWeight: 600,
                              textAlign: "center"
                            }}
                          >
                  ...
                  <div style={{ marginBottom: 24 }}>
                    ...
                    <input
                      ...
                      placeholder="Ej: Factura de mayo..."
                      style={{
                        width: "100%",
                        border: `1.5px solid ${C.border}`,
                        borderRadius: 12,
                  ...
                  <button
                    ...
                    disabled={!selectedGroup || !amount || loading}
                    style={{
                      width: "100%",
                      background: C.accent,
                      color: "white",
...
function DonutChart({ data, size = 160 }) {
  ...
  const slices = data.reduce((acc, d) => {
    ...
    return [
      ...acc,
      { ...d, dash, rotate: prev * 360 - 90, nextOffset: prev + pct }
    ];
  }, []);
...
function BarChart({ currentData, prevData, groups, C }) {
  const allGroupIds = [
    ...new Set([...currentData.map((d) => d.id), ...prevData.map((d) => d.id)])
  ];
    ...
    <div style={{ overflowX: "auto" }}>
      <div
        style={{
          ...
          gap: 16,
          alignItems: "flex-end",
          minWidth: allGroupIds.length * 80,
          padding: "8px 4px 0"
        }}
      >
        {allGroupIds.map((id) => {
              ...
              <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: maxH }}>
                {prev > 0 && (
                  <div
                    style={{
                      width: 14,
                      height: Math.max((prev / maxVal) * maxH, 4),
                      background: group.color + "55",
                      borderRadius: "4px 4px 0 0",
                      transition: "height 0.5s"
                    }}
                    title={`Mes anterior: ${formatAmount(prev)}`}
                  />
                )}
                <div
                  style={{
                    width: 14,
                    height: Math.max((cur / maxVal) * maxH, 4),
                    background: group.color,
                    borderRadius: "4px 4px 0 0",
                    transition: "height 0.5s"
                  }}
                  title={`Mes actual: ${formatAmount(cur)}`}
                />
              </div>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 12, justifyContent: "center" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <div style={{ width: 10, height: 10, background: C.accent + "55", borderRadius: 2 }} />
          <span style={{ fontSize: 11, color: C.muted }}>Mes anterior</span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
          <div style={{ width: 10, height: 10, background: C.accent, borderRadius: 2 }} />
          <span style={{ fontSize: 11, color: C.muted }}>Mes actual</span>
        </div>
      </div>
    </div>
  );
...
function HistoryModal({ onClose, allExpenses, groups, categories, C }) {
  const monthKeys = useMemo(() => {
    const keys = [...new Set(allExpenses.map((e) => getMonthKey(e.date)))]
      .sort()
      .reverse();
              ...
              <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
                {monthKeys.map((k) => (
                  <button
                    ...
                    style={{
                      background: selectedMonth === k ? C.accent : C.card,
                      border: `1px solid ${
                        selectedMonth === k ? C.accent : C.border
                      }`,
                      borderRadius: 10,
                      padding: "8px 14px",
                      cursor: "pointer",
                      color: selectedMonth === k ? "white" : C.muted,
                      fontSize: 13,
                      fontWeight: 600,
                      whiteSpace: "nowrap",
                      flexShrink: 0
                        ...
                        >
                          <div
                            style={{
                              width: 38,
                              height: 38,
                              background: (group?.color || C.accent) + "22",
                              borderRadius: 10,
                              display: "flex",
                              alignItems: "center",
                              justifyContent: "center",
...
function AdminDashboard({
  ...
  const curExpenses = useMemo(
    () => expenses.filter((e) => getMonthKey(e.date) === mk),
    [expenses, mk]
  );

  const prevExpenses = useMemo(
    () => expenses.filter((e) => getMonthKey(e.date) === pmk),
    [expenses, pmk]
  );
  ...
  const excludedGroupIds = new Set(
    groups.filter((g) => g.exclude_from_ranking).map((g) => g.id)
  );

  const byUser = ALL_USERS.map((u) => {
    const rankingExp = curExpenses.filter(
      (e) => e.user_id === u.id && !excludedGroupIds.has(e.group_id)
    );
    return {
      ...u,
      total: rankingExp.reduce((s, e) => s + e.amount, 0),
      count: rankingExp.length
    };
  })
    .filter((u) => u.total > 0)
    .sort((a, b) => b.total - a.total);
  ...
  return (
    ...
    >
      ...
      <div className="admin-layout" style={{ minHeight: "100vh" }}>
        <div
          ...
          style={{
            ...
            top: 0,
            height: "100vh",
            boxShadow: isDark ? "none" : "2px 0 12px #0000000A"
          }}
        >
          ...
          {NAV.map((t) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                marginBottom: 2,
                background: activeSection === t.id ? C.accentSoft : "transparent",
                color: activeSection === t.id ? C.accent : C.muted,
        ...
        >
          ...
          >
            <div>
              ...
              >
                Hola, {currentProfile.name} {currentProfile.avatar}
              </div>
          ...
          {activeSection === "dashboard" && (
            <div>
              <div
                className="stats-grid mobile-stats-grid"
                style={{
                  display: "grid",
                    ...
                    <div style={{ flex: 1, minWidth: 120 }}>
                      {byGroupCur.slice(0, 6).map((g) => (
                        ...
                        >
                          ...
                          >
                            {total > 0 ? Math.round((g.total / total) * 100) : 0}%
                          </span>
                        </div>
                      ))}
                    </div>
              ...
              >
                ...
                >
                  Categorías: este mes vs anterior
                </h3>
                <div style={{ color: C.muted, fontSize: 11, marginBottom: 16 }}>
                  {monthLabel(pmk)} → {monthLabel(mk)}
                </div>
              ...
              >
                ...
                >
                  ...
                  {excludedGroupIds.size > 0 && (
                    ...
                    >
                      {excludedGroupIds.size} categ. excluida
                      {excludedGroupIds.size > 1 ? "s" : ""}
                    </span>
                  )}
                </div>
                  ...
                  byUser.map((u, i) => (
                    <div
                      ...
                      style={{
                        ...
                        gap: 12,
                        padding: "10px 0",
                        borderBottom: i < byUser.length - 1 ? `1px solid ${C.border}` : "none"
                      }}
                    >
                      <div
                        style={{
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          <div
                            style={{
                              height: "100%",
                              width: `${rankingTotal > 0 ? (u.total / rankingTotal) * 100 : 0}%`,
                              background: u.color,
                              borderRadius: 2
                            }}
                          />
          ...
          {activeSection === "gastos" && (
            <div>
              ...
              >
                ...
                >
                  Gastos de {monthLabel(mk)}
                </h2>
                <span style={{ fontSize: 13, color: C.muted }}>
                  {curExpenses.length} registros · {formatAmount(total)}
                </span>
              </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        style={{
                          fontSize: 11,
                          color: u.role === "admin" ? C.accent : C.green,
                          fontWeight: 700,
                          textTransform: "uppercase",
                          letterSpacing: 1,
                          marginBottom: 10
                      ...
                      >
                        <div
                          style={{
                            height: "100%",
                            width: `${total > 0 ? (uTotal / total) * 100 : 0}%`,
                            background: u.color,
                            borderRadius: 2
                          }}
                        />
      ...
      <button
        className="fab-desktop btn-hover"
        onClick={onAdd}
        style={{
          position: "fixed",
          bottom: 32,
...
function UserPanel({
  ...
  const myExpenses = useMemo(
    () =>
      expenses
        .filter((e) => e.user_id === currentProfile.id && getMonthKey(e.date) === mk)
        .sort((a, b) => new Date(b.created_at) - new Date(a.created_at)),
    [expenses, currentProfile.id, mk]
  );
  ...
  return (
    ...
    >
      ...
      <div
        className="app-mobile-bottom-space safe-top"
        style={{ maxWidth: 520, margin: "0 auto", padding: "0 0 24px" }}
      >
        <div
          style={{
            ...
            background: C.surface,
            borderBottom: `1px solid ${C.border}`,
            boxShadow: isDark ? "none" : "0 1px 8px #0000000A"
          }}
        >
          <div
            style={{
          ...
          <div
            style={{
              background: `linear-gradient(135deg, ${currentProfile.color}22, ${C.card})`,
          ...
          >
            ...
            <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>
              {myExpenses.length} registros · {monthLabel(mk)}
            </div>
          </div>
        </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      gap: 12,
                      padding: "14px 16px",
                      borderBottom: i < myExpenses.length - 1 ? `1px solid ${C.border}` : "none",
                      background: "transparent"
                    }}
                  >
                    <div
...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session?.user?.email) {
        setCurrentProfile(USER_PROFILES[session.user.email] || null);
      }
    });
    ...
    return (
      ...
      >
        ...
        <p style={{ marginBottom: 16, color: C.muted }}>
          Este email no está registrado en la familia.
        </p>
        <button
  ...
  if (loading && expenses.length === 0 && groups.length === 0) {
    return (
      <div
        style={{
          minHeight: "100vh",
