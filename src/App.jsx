// src/App.jsx — v4: editar/borrar gastos + fecha automática
...
const DARK = { bg: "#0F0F13", surface: "#17171E", card: "#1E1E28", border: "#2A2A38", accent: "#df2170", accentSoft: "#6C63FF22", green: "#2ECC8A", greenSoft: "#2ECC8A18", red: "#FF5C7A", text: "#F0F0FF", muted: "#8888AA", yellow: "#FFD166" };
const LIGHT = { bg: "#F2F2F7", surface: "#FFFFFF", card: "#FFFFFF", border: "#E0E0EA", accent: "#4f0c28", accentSoft: "#6C63FF15", green: "#1AAE6F", greenSoft: "#1AAE6F15", red: "#E8334A", text: "#0F0F1A", muted: "#8888AA", yellow: "#B8860B" };
function getColors(dark) { return dark ? DARK : LIGHT; }

function getGlobalStyles(C, isDark) {
  return `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .expense-actions { display: flex; gap: 4; flex-shrink: 0; }
  .safe-top { padding-top: max(16px, env(safe-area-inset-top)); }
  .app-mobile-bottom-space { padding-bottom: calc(110px + env(safe-area-inset-bottom)); }
  ...
  @media (max-width: 899px) {
    ...
    .mobile-header { gap: 12px; align-items: flex-start !important; }
    .mobile-header-actions { display: flex; gap: 8px; flex-wrap: wrap; justify-content: flex-end; }
...
// ─── CONSTANTS ────────────────────────────────────────────────────────────────
const USER_PROFILES = {
  "loremalamud@gmail.com":       { id: "lore",  name: "Lore",  role: "admin", color: "#CE93D8", avatar: "👩" },
  "pabloschvartz@gmail.com":     { id: "pablo", name: "Pablo", role: "admin", color: "#80DEEA", avatar: "👨" },
  "matiasschvartz2324@gmail.com":{ id: "mati",  name: "Mati",  role: "user",  color: "#91ffca", avatar: "🧑" },
  "valentinaschvartz@gmail.com": { id: "valen", name: "Valen", role: "admin", color: "#d6a5cf", avatar: "👩" },
};
...
function getUserById(id) { return ALL_USERS.find(u => u.id === id) || { name: "?", color: "#8888AA", avatar: "👤" }; }
...
function prevMonthKey() { const d = new Date(); d.setMonth(d.getMonth() - 1); return getMonthKey(d); }
function monthLabel(key) { if (!key) return ""; const [y, m] = key.split("-"); return `${MONTHS_ES[parseInt(m) - 1]} ${y}`; }
function daysInMonth(key) { const [y, m] = key.split("-").map(Number); return new Date(y, m, 0).getDate(); }
function todayISO() { return new Date().toISOString().split("T")[0]; }

const MONTHLY_ALERT_THRESHOLD = 15000000;
const ADMIN_ALERT_EMAILS = [
  "loremalamud@gmail.com",
  "pabloschvartz@gmail.com",
  "valentinaschvartz@gmail.com",
];
...
function LoginScreen() {
  const isDark = useColorScheme(); const C = getColors(isDark);
  const [email, setEmail] = useState(""); const [password, setPassword] = useState("");
  const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const [showPass, setShowPass] = useState(false);
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div className="card-shadow" style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 20, padding: "28px 24px" }}>
          <div style={{ marginBottom: 16 }}>
            ...
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleLogin()} placeholder="tu@email.com" style={{ width: "100%", border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "13px 16px", fontSize: 15 }} />
          </div>
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ position: "relative" }}>
              <input type={showPass ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleLogin()} placeholder="••••••••" style={{ width: "100%", border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "13px 44px 13px 16px", fontSize: 15 }} />
          ...
          <button className="btn-hover" onClick={handleLogin} disabled={!email || !password || loading} style={{ width: "100%", background: C.accent, color: "white", border: "none", padding: "15px", borderRadius: 12, fontSize: 16, fontWeight: 700 }}>{loading ? "Entrando..." : "Entrar →"}</button>
        </div>
...
// ─── EXPENSE MODAL (crear + editar) ──────────────────────────────────────────
function ExpenseModal({ onClose, onSave, onDelete, currentProfile, groups, categories, editingExpense }) {
  const isDark = useColorScheme(); const C = getColors(isDark);
  ...
  useEffect(() => {
    if (editingExpense) {
      setSelectedGroup(groups.find(g => g.id === editingExpense.group_id) || null);
      setSelectedCat(categories.find(c => c.id === editingExpense.category_id) || null);
  ...
  async function handleSubmit() {
    ...
    await onSave({
      ...
      user_id: isEditing ? editingExpense.user_id : currentProfile.id,
      group_id: selectedGroup.id,
      category_id: selectedCat?.id || null,
      category: selectedCat ? `${selectedGroup.name} › ${selectedCat.name}` : selectedGroup.name,
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#00000099", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.surface, borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 520, border: `1px solid ${C.border}`, borderBottom: "none", maxHeight: "92vh", display: "flex", flexDirection: "column" }}>

        {done ? (
          <div style={{ textAlign: "center", padding: "44px 20px" }}>
            <div style={{ fontSize: 56, marginBottom: 12 }}>{isEditing ? "✏️" : "✅"}</div>
            <p style={{ color: C.green, fontWeight: 600, fontSize: 18 }}>{isEditing ? "¡Gasto actualizado!" : "¡Guardado!"}</p>
          </div>

        ) : deleteConfirm ? (
          <div style={{ padding: "32px 24px", textAlign: "center" }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>🗑️</div>
            <p style={{ color: C.text, fontWeight: 700, fontSize: 18, marginBottom: 8 }}>¿Borrar este gasto?</p>
            <p style={{ color: C.muted, fontSize: 14, marginBottom: 28 }}>
              {selectedGroup?.name} · {formatAmount(parseFloat(amount) || 0)} · {date}
            </p>
            <div style={{ display: "flex", gap: 12 }}>
              <button onClick={() => setDeleteConfirm(false)} style={{ flex: 1, background: C.card, border: `1px solid ${C.border}`, color: C.muted, padding: "14px", borderRadius: 12, fontSize: 15, fontWeight: 600, cursor: "pointer" }}>Cancelar</button>
              <button className="btn-hover" onClick={handleDelete} disabled={loading} style={{ flex: 1, background: C.red, color: "white", border: "none", padding: "14px", borderRadius: 12, fontSize: 15, fontWeight: 700, cursor: "pointer" }}>{loading ? "Borrando..." : "Sí, borrar"}</button>
            </div>
          </div>

        ) : (
          <>
            <div style={{ padding: "22px 24px 14px", borderBottom: `1px solid ${C.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", flexShrink: 0 }}>
              <h2 style={{ color: C.text, fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 20 }}>
                {isEditing ? "✏️ Editar gasto" : "Nuevo gasto"}
              </h2>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                {isEditing && (
                  <button onClick={() => setDeleteConfirm(true)} style={{ background: C.red + "18", border: `1px solid ${C.red}44`, color: C.red, padding: "6px 12px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 600 }}>🗑️ Borrar</button>
                )}
            ...
            <div style={{ overflowY: "auto", flex: 1, padding: "20px 24px 32px" }}>
              {groups.length === 0 ? (
                ...
                {/* Fecha — auto-cargada con hoy, editable */}
                <div style={{ marginBottom: 14 }}>
                  <label style={{ display: "block", color: C.muted, fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>
                    Fecha <span style={{ fontSize: 10, fontWeight: 400, color: C.green }}>✓ hoy por defecto</span>
                  </label>
                ...
                {/* Nota */}
                <div style={{ marginBottom: 24 }}>
                  <label style={{ display: "block", color: C.muted, fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>Nota <span style={{ fontWeight: 400 }}>(opcional)</span></label>
...
// ─── EXPENSE ROW ──────────────────────────────────────────────────────────────
function ExpenseRow({ expense, groups, categories, C, onEdit, onDelete, showUser = false }) {
  const [confirmDelete, setConfirmDelete] = useState(false);
  const group = groups.find(g => g.id === expense.group_id);
  const cat = categories.find(c => c.id === expense.category_id);
  const user = getUserById(expense.user_id);

  if (confirmDelete) {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", background: C.red + "10" }}>
        <span style={{ flex: 1, fontSize: 13, color: C.muted }}>¿Borrar este gasto?</span>
        <button onClick={() => onDelete(expense.id)} style={{ background: C.red, border: "none", color: "white", padding: "6px 14px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 700 }}>Borrar</button>
        <button onClick={() => setConfirmDelete(false)} style={{ background: C.card, border: `1px solid ${C.border}`, color: C.muted, padding: "6px 12px", borderRadius: 8, cursor: "pointer", fontSize: 13 }}>No</button>
      </div>
    );
  }

  return (
    <div className="expense-row row-hover" style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", background: "transparent", transition: "background 0.15s" }}>
      <div style={{ width: 38, height: 38, background: (group?.color || C.accent) + "22", borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 19, flexShrink: 0 }}>
        {cat?.icon || group?.icon || "📦"}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, fontSize: 13, color: C.text }}>{group?.name}{cat ? ` › ${cat.name}` : ""}</div>
        <div style={{ fontSize: 11, color: C.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {showUser && `${user.avatar} ${user.name}`}{showUser && expense.note ? " · " : ""}{expense.note || ""}
        </div>
        <div style={{ fontSize: 10, color: C.muted }}>{expense.date}</div>
      </div>
      <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, color: C.red, fontSize: 14, flexShrink: 0, marginRight: 6 }}>{formatAmount(expense.amount)}</div>
      <div className="expense-actions">
        <button onClick={() => onEdit(expense)} title="Editar" style={{ background: C.accentSoft, border: "none", color: C.accent, width: 30, height: 30, borderRadius: 8, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>✏️</button>
        <button onClick={() => setConfirmDelete(true)} title="Borrar" style={{ background: C.red + "18", border: "none", color: C.red, width: 30, height: 30, borderRadius: 8, cursor: "pointer", fontSize: 14, display: "flex", alignItems: "center", justifyContent: "center" }}>🗑️</button>
      </div>
    </div>
  );
}
...
function ManageCategoriesModal({ onClose, groups, categories, onRefresh }) {
  ...
  const [selectedGroup, setSelectedGroup] = useState(null);
  const [groupName, setGroupName] = useState(""); const [groupIcon, setGroupIcon] = useState("📁"); const [groupColor, setGroupColor] = useState(PALETTE[0]);
  const [catName, setCatName] = useState(""); const [catIcon, setCatIcon] = useState("📦");
  const [loading, setLoading] = useState(false); const [deleteConfirm, setDeleteConfirm] = useState(null);

  async function saveGroup() {
    if (!groupName.trim()) return; setLoading(true);
  ...
  async function saveCat() {
    if (!catName.trim() || !selectedGroup) return; setLoading(true);
  ...
  async function deleteGroup(id) { setLoading(true); await supabase.from("category_groups").delete().eq("id", id); await onRefresh(); setDeleteConfirm(null); setLoading(false); }
  async function deleteCat(id) { setLoading(true); await supabase.from("categories").delete().eq("id", id); await onRefresh(); setDeleteConfirm(null); setLoading(false); }
    ...
    <div style={{ position: "fixed", inset: 0, background: "#000000BB", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.surface, borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 560, border: `1px solid ${C.border}`, borderBottom: "none", maxHeight: "85vh", display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "20px 24px 32px" }}>
          {view === "list" && (
            <div>
              <button className="btn-hover" onClick={() => setView("newGroup")} style={{ width: "100%", background: C.accentSoft, border: `1.5px dashed ${C.accent}`, borderRadius: 12, padding: "14px", color: C.accent, fontWeight: 600, fontSize: 14, marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}><span style={{ fontSize: 18 }}>+</span> Nueva categoría</button>
              ...
              {groups.map(g => {
                ...
                async function toggleExclude() { await supabase.from("category_groups").update({ exclude_from_ranking: !g.exclude_from_ranking }).eq("id", g.id); await onRefresh(); }
                return (
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 16px", borderBottom: gCats.length > 0 ? `1px solid ${C.border}` : "none" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 2 }}>
                          <div style={{ fontSize: 11, color: C.muted }}>{gCats.length} sub{gCats.length !== 1 ? "s" : ""}</div>
                          {g.exclude_from_ranking && <span style={{ fontSize: 10, background: C.yellow + "22", color: C.yellow, border: `1px solid ${C.yellow}44`, borderRadius: 4, padding: "1px 6px", fontWeight: 600 }}>excluida</span>}
                        </div>
                      </div>
                      <button onClick={toggleExclude} title={g.exclude_from_ranking ? "Incluir en ranking" : "Excluir del ranking"} style={{ background: g.exclude_from_ranking ? C.yellow + "22" : C.card, border: `1px solid ${g.exclude_from_ranking ? C.yellow : C.border}`, color: g.exclude_from_ranking ? C.yellow : C.muted, borderRadius: 8, padding: "5px 8px", fontSize: 14, cursor: "pointer", marginRight: 4 }}>🏆</button>
              ...
              {groups.length > 0 && <div style={{ background: C.accentSoft, border: `1px solid ${C.accent}33`, borderRadius: 10, padding: "10px 14px", marginTop: 4, fontSize: 12, color: C.muted, lineHeight: 1.5 }}>💡 Tocá <strong>🏆</strong> para excluir una categoría del ranking.</div>}
            </div>
          )}
          {view === "newGroup" && (
            <div>
              <div style={{ marginBottom: 20 }}>
                ...
                <input value={groupName} onChange={e => setGroupName(e.target.value)} placeholder="Ej: Casa, Auto, Salud..." style={{ width: "100%", border: `1.5px solid ${C.border}`, borderRadius: 12, padding: "13px 16px", fontSize: 16 }} />
              </div>
              <div style={{ marginBottom: 20 }}>
...
function DonutChart({ data, size = 160 }) {
  ...
  const slices = data.reduce((acc, d) => {
    const pct = d.value / total, len = pct * (2 * Math.PI * r), dash = `${len} ${2 * Math.PI * r - len}`;
    const prev = acc.length > 0 ? acc[acc.length - 1].nextOffset : 0;
    return [...acc, { ...d, dash, rotate: prev * 360 - 90, nextOffset: prev + pct }];
  }, []);
  return (
    <svg width={size} height={size} viewBox="0 0 160 160">
      {slices.map((s, i) => <circle key={i} cx={cx} cy={cy} r={r} fill="none" stroke={s.color} strokeWidth="22" strokeDasharray={s.dash} strokeDashoffset="0" style={{ transform: `rotate(${s.rotate}deg)`, transformOrigin: `${cx}px ${cy}px` }} />)}
...
function BarChart({ currentData, prevData, groups, C }) {
  ...
  const maxVal = Math.max(...allGroupIds.flatMap(id => { const cur = currentData.find(d => d.id === id)?.total || 0; const prev = prevData.find(d => d.id === id)?.total || 0; return [cur, prev]; }), 1);
  return (
    <div style={{ overflowX: "auto" }}>
      <div style={{ display: "flex", gap: 16, alignItems: "flex-end", minWidth: allGroupIds.length * 80, padding: "8px 4px 0" }}>
        {allGroupIds.map(id => {
          ...
          const cur = currentData.find(d => d.id === id)?.total || 0, prev = prevData.find(d => d.id === id)?.total || 0, maxH = 100;
          return (
            <div key={id} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6, minWidth: 60 }}>
              <div style={{ display: "flex", alignItems: "flex-end", gap: 4, height: maxH }}>
                {prev > 0 && <div style={{ width: 14, height: Math.max((prev / maxVal) * maxH, 4), background: group.color + "55", borderRadius: "4px 4px 0 0" }} title={`Mes anterior: ${formatAmount(prev)}`} />}
                <div style={{ width: 14, height: Math.max((cur / maxVal) * maxH, 4), background: group.color, borderRadius: "4px 4px 0 0" }} title={`Mes actual: ${formatAmount(cur)}`} />
              </div>
...
function LineChart({ expenses, monthKey, C }) {
  ...
  const max = Math.max(...dailyTotals, 1), W = 280, H = 80, pad = 8;
  const points = dailyTotals.map((v, i) => { const x = pad + (i / Math.max(days - 1, 1)) * (W - pad * 2), y = H - pad - (v / max) * (H - pad * 2); return `${x},${y}`; }).join(" ");
  if (!dailyTotals.some(v => v > 0)) return <div style={{ color: C.muted, textAlign: "center", padding: "30px 0", fontSize: 13 }}>Sin gastos este mes</div>;
  return (
    <div style={{ overflowX: "auto" }}>
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ minWidth: 200 }}>
        ...
        {dailyTotals.map((v, i) => { if (v === 0) return null; const x = pad + (i / Math.max(days - 1, 1)) * (W - pad * 2), y = H - pad - (v / max) * (H - pad * 2); return <circle key={i} cx={x} cy={y} r="3" fill={C.accent} />; })}
      </svg>
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
        <span style={{ fontSize: 10, color: C.muted }}>1</span><span style={{ fontSize: 10, color: C.muted }}>{Math.ceil(days / 2)}</span><span style={{ fontSize: 10, color: C.muted }}>{days}</span>
      </div>
    </div>
  );
}
...
// ─── HISTORY MODAL ────────────────────────────────────────────────────────────
function HistoryModal({ onClose, allExpenses, groups, categories, C, onEdit, onDelete }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#000000BB", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 1000, backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background: C.surface, borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 600, border: `1px solid ${C.border}`, borderBottom: "none", maxHeight: "88vh", display: "flex", flexDirection: "column" }}>
        ...
        {monthKeys.length === 0 ? (
          <div style={{ textAlign: "center", padding: "40px 20px", color: C.muted }}><div style={{ fontSize: 40, marginBottom: 12 }}>📭</div><p>No hay historial todavía.</p></div>
        ) : (
          <>
            <div style={{ padding: "16px 24px", borderBottom: `1px solid ${C.border}`, flexShrink: 0 }}>
              <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 4 }}>
                {monthKeys.map(k => <button key={k} onClick={() => setSelectedMonth(k)} style={{ background: selectedMonth === k ? C.accent : C.card, border: `1px solid ${selectedMonth === k ? C.accent : C.border}`, borderRadius: 10, padding: "8px 14px", cursor: "pointer", color: selectedMonth === k ? "white" : C.muted, fontSize: 13, fontWeight: 600, whiteSpace: "nowrap", flexShrink: 0 }}>{monthLabel(k)}</button>)}
              </div>
            </div>
            <div style={{ overflowY: "auto", flex: 1, padding: "20px 24px 32px" }}>
              ...
              {monthExpenses.length === 0 ? <div style={{ textAlign: "center", padding: "20px", color: C.muted }}>No hay gastos en este mes.</div> : (
                <div style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.border}`, overflow: "hidden" }}>
                  {[...monthExpenses].sort((a, b) => new Date(b.date) - new Date(a.date)).map((e, i, arr) => (
                    <div key={e.id} style={{ borderBottom: i < arr.length - 1 ? `1px solid ${C.border}` : "none" }}>
                      <ExpenseRow expense={e} groups={groups} categories={categories} C={C}
                        onEdit={exp => { onClose(); setTimeout(() => onEdit(exp), 100); }}
                        onDelete={onDelete} showUser={true} />
                    </div>
                  ))}
                </div>
              )}
...
// ─── ADMIN DASHBOARD ──────────────────────────────────────────────────────────
function AdminDashboard({ expenses, currentProfile, onAdd, onEdit, onDelete, onLogout, groups, categories, onRefresh }) {
  const isDark = useColorScheme(); const C = getColors(isDark);
  const [showManage, setShowManage] = useState(false);
  const [showHistory, setShowHistory] = useState(false);
  const [activeSection, setActiveSection] = useState("dashboard");

  const mk = currentMonthKey(), pmk = prevMonthKey();
  ...
  const diff = total - prevTotal, diffPct = prevTotal > 0 ? ((diff / prevTotal) * 100).toFixed(0) : null;
  ...
  const byUser = ALL_USERS.map(u => { const r = curExpenses.filter(e => e.user_id === u.id && !excludedGroupIds.has(e.group_id)); return { ...u, total: r.reduce((s, e) => s + e.amount, 0), count: r.length }; }).filter(u => u.total > 0).sort((a, b) => b.total - a.total);
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', sans-serif", color: C.text }}>
      ...
      <div className="admin-layout" style={{ minHeight: "100vh" }}>
        ...
        <div className="admin-sidebar" style={{ display: "none", flexDirection: "column", background: C.surface, borderRight: `1px solid ${C.border}`, padding: "28px 16px", position: "sticky", top: 0, height: "100vh", boxShadow: isDark ? "none" : "2px 0 12px #0000000A" }}>
          <div style={{ marginBottom: 32 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4, padding: "0 8px" }}><span style={{ fontSize: 26 }}>🏡</span><span style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 17, color: C.text }}>Schvartz</span></div>
          ...
          {NAV.map(t => <button key={t.id} className="btn-hover" onClick={() => setActiveSection(t.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 10, border: "none", cursor: "pointer", marginBottom: 2, background: activeSection === t.id ? C.accentSoft : "transparent", color: activeSection === t.id ? C.accent : C.muted, fontWeight: activeSection === t.id ? 600 : 400, fontSize: 14, width: "100%", textAlign: "left" }}><span style={{ fontSize: 18 }}>{t.icon}</span> {t.label}</button>)}
          <button className="btn-hover" onClick={() => setShowHistory(true)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 10, border: "none", cursor: "pointer", marginBottom: 2, background: "transparent", color: C.muted, fontSize: 14, width: "100%", textAlign: "left" }}><span style={{ fontSize: 18 }}>📅</span> Historial</button>
          <button className="btn-hover" onClick={() => setShowManage(true)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 12px", borderRadius: 10, border: "none", cursor: "pointer", background: "transparent", color: C.muted, fontSize: 14, width: "100%", textAlign: "left" }}><span style={{ fontSize: 18 }}>⚙️</span> Categorías</button>
          <div style={{ marginTop: "auto", borderTop: `1px solid ${C.border}`, paddingTop: 16 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12, padding: "0 4px" }}><span style={{ fontSize: 26 }}>{currentProfile.avatar}</span><div><div style={{ fontWeight: 600, fontSize: 14, color: C.text }}>{currentProfile.name}</div><div style={{ fontSize: 11, color: C.accent, fontWeight: 600 }}>Admin</div></div></div>
        ...
        {/* Main */}
        <div className="main-pad app-mobile-bottom-space safe-top" style={{ padding: "20px 20px 150px", overflowY: "auto" }}>
          {/* Mobile header */}
          <div className="mobile-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
            <div>
          ...
          {activeSection === "dashboard" && (<div>
            <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
              {[
                { label: "Total del mes", value: formatAmount(total), sub: diffPct !== null ? `${diff > 0 ? "▲" : "▼"} ${Math.abs(diffPct)}% vs mes anterior` : null, subColor: diff > 0 ? C.red : C.green },
                { label: "Gastos cargados", value: curExpenses.length, sub: "este mes" },
                { label: "Mes anterior", value: formatAmount(prevTotal), sub: monthLabel(pmk) },
                { label: "Categorías", value: groups.length, sub: "configuradas" },
              ].map((s, i) => (
                <div key={i} className="card-shadow" style={{ background: C.card, borderRadius: 16, padding: "16px 18px", border: `1px solid ${C.border}` }}>
                  <div style={{ color: C.muted, fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: 1, marginBottom: 8 }}>{s.label}</div>
                  <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: 26, fontWeight: 700, color: C.text, lineHeight: 1.1 }}>{s.value}</div>
                  {s.sub && <div style={{ fontSize: 12, color: s.subColor || C.muted, marginTop: 4, fontWeight: s.subColor ? 600 : 400 }}>{s.sub}</div>}
                </div>
              ))}
            </div>
            <div className="charts-grid" style={{ marginBottom: 16 }}>
              <div className="card-shadow" style={{ background: C.card, borderRadius: 16, padding: "20px", border: `1px solid ${C.border}`, marginBottom: 16 }}>
                <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 14, marginBottom: 20, color: C.text }}>Distribución por categoría</h3>
            ...
            <div className="card-shadow" style={{ background: C.card, borderRadius: 16, padding: "20px", marginBottom: 16, border: `1px solid ${C.border}` }}>
              <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 14, marginBottom: 4, color: C.text }}>Este mes vs anterior</h3>
            ...
            <div className="card-shadow" style={{ background: C.card, borderRadius: 16, padding: "20px", border: `1px solid ${C.border}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
                ...
                {excludedGroupIds.size > 0 && <span style={{ fontSize: 11, background: C.yellow + "22", color: C.yellow, border: `1px solid ${C.yellow}44`, borderRadius: 6, padding: "2px 8px", fontWeight: 600 }}>{excludedGroupIds.size} excluida{excludedGroupIds.size > 1 ? "s" : ""}</span>}
              </div>
              {byUser.length === 0 ? <div style={{ textAlign: "center", color: C.muted, padding: "16px 0", fontSize: 13 }}>Nadie cargó gastos todavía</div>
                : byUser.map((u, i) => (
                  <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: i < byUser.length - 1 ? `1px solid ${C.border}` : "none" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ height: 4, background: C.border, borderRadius: 2, marginTop: 4, overflow: "hidden" }}><div style={{ height: "100%", width: `${rankingTotal > 0 ? (u.total / rankingTotal) * 100 : 0}%`, background: u.color, borderRadius: 2 }} /></div>
                    </div>
          ...
          {/* GASTOS */}
          {activeSection === "gastos" && (<div>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
              <h2 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, fontSize: 20, color: C.text }}>Gastos de {monthLabel(mk)}</h2>
              <span style={{ fontSize: 13, color: C.muted }}>{curExpenses.length} · {formatAmount(total)}</span>
            </div>
            {curExpenses.length === 0 ? (
              <div className="card-shadow" style={{ background: C.card, borderRadius: 16, padding: "40px", border: `1px solid ${C.border}`, textAlign: "center", color: C.muted }}><div style={{ fontSize: 36, marginBottom: 10 }}>📋</div><p>No hay gastos este mes.</p></div>
            ) : (
              <div className="card-shadow" style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.border}`, overflow: "hidden" }}>
                {[...curExpenses].sort((a, b) => new Date(b.date) - new Date(a.date)).map((e, i, arr) => (
                  <div key={e.id} style={{ borderBottom: i < arr.length - 1 ? `1px solid ${C.border}` : "none" }}>
                    <ExpenseRow expense={e} groups={groups} categories={categories} C={C} onEdit={onEdit} onDelete={onDelete} showUser={true} />
                  </div>
                ))}
              </div>
            )}
          ...
          {activeSection === "miembros" && (<div>
            ...
            <div className="members-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 20 }}>
              {ALL_USERS.map(u => {
                const uExp = curExpenses.filter(e => e.user_id === u.id), uTotal = uExp.reduce((s, e) => s + e.amount, 0);
                return (
                  <div key={u.id} className="card-shadow" style={{ background: C.card, borderRadius: 16, padding: "18px", border: `1px solid ${u.color}33` }}>
                    ...
                    <div style={{ height: 4, background: C.border, borderRadius: 2, marginTop: 10, overflow: "hidden" }}><div style={{ height: "100%", width: `${total > 0 ? (uTotal / total) * 100 : 0}%`, background: u.color, borderRadius: 2 }} /></div>
                  </div>
                );
              })}
            </div>
            {ALL_USERS.map(u => {
              ...
              return (
                <div key={u.id} className="card-shadow" style={{ background: C.card, borderRadius: 16, padding: "18px", marginBottom: 12, border: `1px solid ${C.border}` }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}><span style={{ fontSize: 22 }}>{u.avatar}</span><span style={{ fontWeight: 700, fontSize: 15, color: C.text }}>{u.name}</span></div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{uGroups.map(g => <div key={g.id} style={{ background: g.color + "18", border: `1px solid ${g.color}44`, borderRadius: 8, padding: "4px 10px", fontSize: 12, color: g.color, fontWeight: 500 }}>{g.icon} {g.name} · {formatAmount(g.total)}</div>)}</div>
                </div>
              );
            })}
          </div>)}
      ...
      {/* Bottom nav */}
      <div className="bottom-nav" style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: C.surface, borderTop: `1px solid ${C.border}`, display: "flex", alignItems: "center", padding: "10px 8px 18px", zIndex: 1000 }}>
        {NAV.map(t => <button key={t.id} onClick={() => setActiveSection(t.id)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: "none", border: "none", cursor: "pointer", padding: "6px 0", color: activeSection === t.id ? C.accent : C.muted, fontSize: 10, fontWeight: activeSection === t.id ? 700 : 400 }}><span style={{ fontSize: 20 }}>{t.icon}</span>{t.label}</button>)}
        <button onClick={() => setShowHistory(true)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: "none", border: "none", cursor: "pointer", padding: "6px 0", color: C.muted, fontSize: 10 }}><span style={{ fontSize: 20 }}>📅</span>Historial</button>
        <button onClick={() => setShowManage(true)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 2, background: "none", border: "none", cursor: "pointer", padding: "6px 0", color: C.muted, fontSize: 10 }}><span style={{ fontSize: 20 }}>⚙️</span>Categ.</button>
        <button onClick={onAdd} style={{ background: C.accent, border: "none", borderRadius: "50%", width: 44, height: 44, fontSize: 22, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: `0 6px 18px ${C.accent}66`, flexShrink: 0, color: "white", marginLeft: 6 }}>+</button>
      </div>
      <button className="fab-desktop btn-hover" onClick={onAdd} style={{ position: "fixed", bottom: 32, right: 32, background: C.accent, border: "none", borderRadius: 16, padding: "14px 24px", fontSize: 15, fontWeight: 700, cursor: "pointer", display: "none", alignItems: "center", gap: 8, boxShadow: `0 4px 24px ${C.accent}66`, color: "white", zIndex: 50 }}>
      ...
      {showHistory && <HistoryModal onClose={() => setShowHistory(false)} allExpenses={expenses} groups={groups} categories={categories} C={C}
        onEdit={exp => { setShowHistory(false); setTimeout(() => onEdit(exp), 100); }} onDelete={onDelete} />}
    </div>
  );
}

// ─── USER PANEL ───────────────────────────────────────────────────────────────
function UserPanel({ expenses, currentProfile, onAdd, onEdit, onDelete, onLogout, groups, categories, onRefresh }) {
  const isDark = useColorScheme(); const C = getColors(isDark);
  const [showManage, setShowManage] = useState(false);
  const [showHistory, setShowHistory] = useState(false);
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: "'DM Sans', sans-serif", color: C.text }}>
      <style>{getGlobalStyles(C, isDark)}</style>
      <div className="app-mobile-bottom-space safe-top" style={{ maxWidth: 520, margin: "0 auto", padding: "0 0 24px" }}>
        <div style={{ padding: "28px 20px 20px", background: C.surface, borderBottom: `1px solid ${C.border}` }}>
        ...
        <div style={{ padding: "4px 20px 40px" }}>
          <h3 style={{ fontFamily: "'Space Grotesk', sans-serif", fontWeight: 700, marginBottom: 12, color: C.muted, textTransform: "uppercase", letterSpacing: 1, fontSize: 11 }}>MIS GASTOS</h3>
          {myExpenses.length === 0 ? (
            <div style={{ textAlign: "center", padding: "40px 20px", color: C.muted }}><div style={{ fontSize: 48, marginBottom: 12 }}>🌱</div><p>Todavía no cargaste gastos este mes.</p></div>
          ) : (
            <div className="card-shadow" style={{ background: C.card, borderRadius: 16, border: `1px solid ${C.border}`, overflow: "hidden" }}>
              {myExpenses.map((e, i) => (
                <div key={e.id} style={{ borderBottom: i < myExpenses.length - 1 ? `1px solid ${C.border}` : "none" }}>
                  <ExpenseRow expense={e} groups={groups} categories={categories} C={C} onEdit={onEdit} onDelete={onDelete} showUser={false} />
                </div>
              ))}
            </div>
          )}
...
export default function App() {
  ...
  async function fetchAll() {
    ...
    setExpenses(exp || []); setGroups(grp || []); setCategories(cat || []);
    setLoading(false);
  }
...
async function checkAndSendBudgetAlert() {
  ...
  const familyUserIds = ALL_USERS.map((u) => u.id);
  ...
  if (error) {
    console.error("Error checking monthly total:", error);
    return;
  }
  ...
  if (monthlyTotal <= MONTHLY_ALERT_THRESHOLD) return;
  ...
  const { data, error: fnError } = await supabase.functions.invoke(
    "send-budget-alert",
    {
      body: {
        monthKey,
        threshold: MONTHLY_ALERT_THRESHOLD,
        total: monthlyTotal,
        adminEmails: ADMIN_ALERT_EMAILS,
      },
    }
  );

  if (fnError) {
    console.error("Error invoking send-budget-alert:", fnError);
    return;
  }

  console.log("Budget alert result:", data);
}
 ...
 async function handleSave(expense) {
  ...
  function handleEdit(expense) { setEditingExpense(expense); setShowModal(true); }
  function handleAdd() { setEditingExpense(null); setShowModal(true); }
  function handleCloseModal() { setShowModal(false); setEditingExpense(null); }
  ...
  if (loading && expenses.length === 0 && groups.length === 0) return (
    <div style={{ minHeight: "100vh", background: C.bg, display: "flex", alignItems: "center", justifyContent: "center", color: C.muted, fontFamily: "'DM Sans', sans-serif" }}>
      <style>{getGlobalStyles(C, isDark)}</style>Cargando...
    </div>
  );
  ...
  return (
    <>
      {currentProfile.role === "admin"
        ? <AdminDashboard expenses={expenses} currentProfile={currentProfile} onAdd={handleAdd} onEdit={handleEdit} onDelete={handleDelete} onLogout={handleLogout} groups={groups} categories={categories} onRefresh={fetchAll} />
        : <UserPanel expenses={expenses} currentProfile={currentProfile} onAdd={handleAdd} onEdit={handleEdit} onDelete={handleDelete} onLogout={handleLogout} groups={groups} categories={categories} onRefresh={fetchAll} />}
      {showModal && (
        <ExpenseModal onClose={handleCloseModal} onSave={handleSave} onDelete={handleDelete} currentProfile={currentProfile} groups={groups} categories={categories} editingExpense={editingExpense} />
      )}
    </>
  );
}